...
def run_simulation(
    ...
    with open(csv_file, "w", newline="") as f:
        ...
        avg_wait = (
            sum(stats.wait_times) / len(stats.wait_times) if stats.wait_times else 0
        )
...
def cmd_apply(args: argparse.Namespace) -> None:
    ...
    rng = random.Random(
        args.seed if args.seed is not None else scheduler_params.get("seed", 42)
    )
    workload_events = build_workload_from_config(workload_cfg, rng)
...
def main() -> None:
    ...
    parser_run = subparsers.add_parser(
        "run", help="Run experiment from a single YAML config"
    )
    parser_run.add_argument(
        "--config", "-c", required=True, help="Path to the experiment YAML file"
    )
    parser_run.add_argument(
    ...
    parser_apply = subparsers.add_parser(
        "apply", help="Apply simulation with separate configs"
    )
    parser_apply.add_argument(
        "--cluster", "-f", required=True, help="Path to cluster YAML configuration"
    )
    parser_apply.add_argument(
        "--scheduler", "-s", required=True, help="Path to scheduler YAML configuration"
    )
    parser_apply.add_argument(
        "--workload", "-w", required=True, help="Path to workload YAML configuration"
    )
    parser_apply.add_argument(
        "--output", "-o", default="results", help="Output directory (default: results/)"
    )
